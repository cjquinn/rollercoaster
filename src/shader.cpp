...
bool Shader::loadShader(std::string file, int type)
{
    ...
    if (status == GL_FALSE) {
        ...
        if (type == GL_VERTEX_SHADER) {
            sprintf(shader_type, "vertex shader");
        } else if (type == GL_FRAGMENT_SHADER) {
            sprintf(shader_type, "fragment shader");
        } else if (type == GL_GEOMETRY_SHADER) {
            sprintf(shader_type, "geometry shader");
        } else {
            sprintf(shader_type, "unknown shader type");
        }

        sprintf(message, "Error in %s!\n%s\nShader file not compiled.  The compiler returned:\n\n%s", shader_type, file.c_str(), log);
